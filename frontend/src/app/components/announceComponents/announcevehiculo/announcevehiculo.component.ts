...
@Component({
  selector: 'app-announcevehiculo',
  templateUrl: './announcevehiculo.component.html',
  styleUrls: ['./announcevehiculo.component.css']
})

export class AnnouncevehiculoComponent {
  ...
  onSubmit() {
    // Aquí es donde colocas la lógica que deseas ejecutar después de presionar el botón submit
  ...
  onSelectedSellerType(event:any){
  ...
  onSelectedUbication(event:any){
    //Mediante esta variable consigo cambiar el estilo de el texto de los elementos select una vez seleccionada la opcion
    this.selectedOptionStyle6 = event.target.value
  }
  ...
  uploadData(brand:HTMLSelectElement, model:HTMLInputElement, price:HTMLInputElement, year:HTMLInputElement, title:HTMLInputElement, sellerType:HTMLSelectElement, version:HTMLInputElement, numberSeats:HTMLInputElement, numberDoors:HTMLInputElement, horsepower:HTMLInputElement, carType:HTMLSelectElement, kilometres:HTMLInputElement, engine:HTMLSelectElement, gearShift:HTMLSelectElement, email:HTMLInputElement, phone:HTMLInputElement, description:HTMLTextAreaElement, ubication:HTMLSelectElement): boolean{//La funcion tiene que retornar un Boolean para poder recoger los inputs (No se porque)
    //VARIABLES
    let objtID:any //Objeto que almacena el ID del anuncio creado (para pasarselo al componente que muestra el anuncio una vez creado)
    let type = "vehicle" //Variable que almacena el tipo de anuncio 
    ...
    if(this.objectPhotosEdit.arrayPhotosEdit != undefined && this.objectPhotosEdit.arrayPhotosEditDelete != undefined && this.objectPhotos.arrayUpload != undefined){ //Compruebo que los arrays no sean undefined para no generar errores
      this.postUpdateServices.uploadAnnounceVehiculo(brand.value, model.value, price.value, year.value, title.value, sellerType.value, version.value, numberSeats.value, numberDoors.value, horsepower.value, carType.value, kilometres.value, engine.value, gearShift.value, email.value, phone.value, description.value, ubication.value, type, editNum, this.itsEditRecibido, _idEdit, this.objectPhotosEdit.arrayPhotosEditDelete, this.objectPhotosEdit.arrayPhotosEdit , this.objectPhotos.arrayUpload, this._idUser)
      .subscribe(
        res => {
