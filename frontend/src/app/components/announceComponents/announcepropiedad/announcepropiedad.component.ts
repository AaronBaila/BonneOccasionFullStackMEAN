...
@Component({
  selector: 'app-announcepropiedad',
  templateUrl: './announcepropiedad.component.html',
  styleUrls: ['./announcepropiedad.component.css']
})

export class AnnouncepropiedadComponent {
  ...
  //Funcion para obtener los archivos y mostrarlos en preview
  ...
  onSelectedRentOrSell(event:any){
    //Mediante esta variable consigo cambiar el estilo de el texto de los elementos select una vez seleccionada la opcion
    this.selectedOptionStyle2 = event.target.value
  }

  onSelectedTypeOfSpace(event:any){
    //Mediante esta variable consigo cambiar el estilo de el texto de los elementos select una vez seleccionada la opcion
  ...
  //Funcion para eliminar foto en preview
  ...
  //Funcion para cambiar foto en preview
  ...
  uploadData(title:HTMLInputElement, sellerType:HTMLSelectElement, rentOrSell:HTMLSelectElement, typeOfSpace:HTMLSelectElement, price:HTMLInputElement, area:HTMLInputElement, status:HTMLSelectElement, email:HTMLInputElement, phone:HTMLInputElement, description:HTMLTextAreaElement, ubication:HTMLSelectElement): boolean{//La funcion tiene que retornar un Boolean para poder recoger los inputs (No se porque)
    //VARIABLES
    ...
    let type = "property" //Variable que almacena el tipo de anuncio 
    ...
    if(this.objectPhotosEdit.arrayPhotosEdit != undefined && this.objectPhotosEdit.arrayPhotosEditDelete != undefined && this.objectPhotos.arrayUpload != undefined){ //Compruebo que los arrays no sean undefined para no generar errores
      this.postUpdateServices.uploadAnnounceProperty(title.value, sellerType.value, rentOrSell.value, typeOfSpace.value, price.value, area.value, status.value, email.value, phone.value, description.value, ubication.value, type, editNum, this.itsEditRecibido, _idEdit, this.objectPhotosEdit.arrayPhotosEditDelete, this.objectPhotosEdit.arrayPhotosEdit, this.objectPhotos.arrayUpload, this._idUser)
      .subscribe(
        res => {
