...
@Component({
  selector: 'app-announceempleo',
  templateUrl: './announceempleo.component.html',
  styleUrls: ['./announceempleo.component.css']
})
export class AnnounceempleoComponent {
  ...
  objectAuxEdit = { //Mediante este objeto auxiliar, almacenaremos todos los datos del anuncio a editar. En caso de ser una publicacion, se inicializa con datos vacios para no provocar un error por "undefined" en el HTML, ya que lo uso para rellenar los inputs con "value"
    _id:"",
    title:"",
    category:"Job Offer or Job Seeking ?",
    price:"",
    email:"",
    phone:"",
    description:"",
  ...
  constructor(private postUpdateServices:PostupdateservicesService, private getServices:GetservicesService, private router:Router, public authService:AuthservicesService, private compService: ComponentservicesService, private deleteServices:DeleteservicesService){ //Instancio el servicio
    //Instanciamos las variables mediante el uso de los objetos auxiliares para no generar errores por undefined
  ...
  ngOnInit() { //Funcion que se va a ejecutar nada mas cargar el componente
    ...
    if(this.itsEditRecibido){ //En caso de que sea una edicion, rellenamos las variables con los datos del anuncio a editar
      this.objectAuxEdit = this.announceRecibido
      this.objectPhotosEdit.arrayPhotosEdit = this.announceRecibido.photosPaths //Obtenemos el path de las fotos del objeto a editar
    }
  ...
  uploadData(title:HTMLInputElement, category:HTMLSelectElement, price:HTMLInputElement, email:HTMLInputElement, phone:HTMLInputElement, description:HTMLTextAreaElement, ubication:HTMLSelectElement): boolean{//La funcion tiene que retornar un Boolean para poder recoger los inputs (No se porque)
    ...
    let type = "job" //Variable que almacena el tipo de anuncio 
