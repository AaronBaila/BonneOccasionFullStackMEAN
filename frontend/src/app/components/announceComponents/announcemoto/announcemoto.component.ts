...
@Component({
  selector: 'app-announcemoto',
  templateUrl: './announcemoto.component.html',
  styleUrls: ['./announcemoto.component.css']
})
export class AnnouncemotoComponent {
  ...
  uploadData(brand:HTMLInputElement, model:HTMLInputElement, price:HTMLInputElement, year:HTMLInputElement, title:HTMLInputElement, sellerType:HTMLSelectElement, version:HTMLInputElement, horsepower:HTMLInputElement, motoType:HTMLSelectElement, kilometres:HTMLInputElement, email:HTMLInputElement, phone:HTMLInputElement, description:HTMLTextAreaElement, ubication:HTMLSelectElement): boolean{//La funcion tiene que retornar un Boolean para poder recoger los inputs (No se porque)
    //VARIABLES
    ...
    let type = "moto" //Variable que almacena el tipo de anuncio 
    ...
    if(this.objectPhotosEdit.arrayPhotosEdit != undefined && this.objectPhotosEdit.arrayPhotosEditDelete != undefined && this.objectPhotos.arrayUpload != undefined){ //Compruebo que los arrays no sean undefined para no generar errores
      this.postUpdateServices.uploadAnnounceMoto(brand.value, model.value, price.value, year.value, title.value, sellerType.value, version.value, horsepower.value, motoType.value, kilometres.value, email.value, phone.value, description.value, ubication.value, type, editNum, this.itsEditRecibido, _idEdit, this.objectPhotosEdit.arrayPhotosEditDelete, this.objectPhotosEdit.arrayPhotosEdit , this.objectPhotos.arrayUpload, this._idUser)
      .subscribe(
        res => {
